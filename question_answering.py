...
def main(pdf_path, query_text, relevance_threshold=np.float32(0.6)):
    ...
    try:
        ...
        # Lower the relevance threshold if no chunks are found
    ...
    try:
        # Print context chunks for debugging
        ...
        if isinstance(answer, dict):
           ...
           print(f"Error querying Gemini model: {answer}")
        
        print("\nCited Context:")
        for text, _, idx in answer.get("relevant_context", []):
